(() => {
  ...
  const footerText = (message) => {
    const parts = [
      `⏱️ ${formatMs(message.ttftMs)} 首字`,
      `⚡ ${formatTps(message.outputTokens, message.durationMs, message.ttftMs)}`,
      `⌛ ${formatSeconds(message.durationMs)} 总耗时`,
      `📥 输入: ${formatTokens(message.inputTokens)} tokens`,
      `📤 输出: ${formatTokens(message.outputTokens)} tokens`,
      `🪙 合计: ${formatTokens(message.totalTokens)} tokens`,
    ];
    return parts.join(" · ");
  };
  ...
  const finishOrRequeue = (message, result) => {
    if (message.resolveAttempts >= maxResolveAttempts) {
      // Give up and keep the partial footer rather than polling forever.
  ...
  const trackTurn = (row) => {
    ...
    const recentStart =
      lastTurnStart && lastTurnStart.at != null && now - lastTurnStart.at < 30_000
        ? lastTurnStart
        : null;
    const sessionId = currentSessionId() || recentStart?.threadId || "";
    const message = {
      row,
      turnKey,
      sessionId,
      t0: recentStart?.t0 ?? null,
      firstTokenTs: null,
      ttftMs: null,
    ...
    if (message.t0 == null) {
      ...
      if (pendingRows.length > 8) pendingRows.shift();
    }
  ...
  const onTurnRequest = (event) => {
    ...
    while (pendingRows.length > 0) {
      const message = pendingRows.shift();
      if (!message || message.t0 != null || message.finalized) continue;
      message.t0 = t0;
      if (message.firstTokenTs != null) {
        message.ttftMs = message.firstTokenTs - t0;
      }
    }
  };

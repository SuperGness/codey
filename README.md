...
- 请求日志：可选择记录线路请求，在以明细列表为主体的独立页面中自适应查看首字与总耗时、Token 总量及输入、输出、推理明细、缓存用量与命中率、传输状态，顶部集中展示单行常用筛选与紧凑关键指标，支持收起概览以扩大日志列表视口，并可按需展开趋势与分组统计；提供组合筛选、点击日志行或通过键盘在侧边详情中查看耗时分解与诊断信息、精确查找请求或会话、复制 ID 及清理历史日志。页面会提示记录停止、写入异常、日志丢弃和用量缺失。

...
- 插件线路：启用声明了线路能力的插件后添加一条线路，密钥在线路中填写；停用时删除尚未填写密钥的线路，已填写密钥的线路会保留。
- 任务数量：插件通过 `codey.appserver.v1` 调用 `codey://getTasks`，查询当前正在运行和失败的任务数量；无人调用时不会主动统计。
...
- 发布管理：可在独立 Cloudflare 控制台管理版本、更新日志、设备机器号及全量、白名单灰度和定向发布。

...
- 插件与页面增强：修复插件市场和本地插件展示，提供可离线恢复的精选插件内容，并改善常用会话操作和页面体验。macOS 下为用户主动开启的旧版电脑操作服务保留独立的兼容入口，避免重启后失效；手动关闭兼容入口后不会自动重新开启。
...
- 稳定性与存储保护：提供健康检查、会话恢复、诊断存储统计与清理，并可按平台启用 Trace、Crashpad、宠物精简和渲染诊断选项。
...
- 开启本地路由后，若任务尚未完成线路切换，会暂停发送并提示重新打开任务，避免请求发往其他供应商。

...
- 请求日志：可选择记录线路请求，在独立页面按时间范围查看请求量、成功率、平均耗时及 Token 用量，并按线路、实际模型、请求类型、会话等条件筛选、分组和查看趋势；支持逐页查看明细、精确查找请求或会话、复制 ID、查看错误摘要及清理历史日志。页面会提示记录停止、写入异常、日志丢弃和用量缺失。
...
- 连接异常后的后续工具调用会在可恢复完整历史时继续；历史缺失或身份变化时明确提示补充完整上下文，避免继续提交缺少对应调用的工具结果。
- 部分增强能力会随 Codex 版本和所选线路而变化，请以控制台提示为准。
- 请求日志用于日常诊断。异常退出、存储故障或请求过多时可能缺失记录；上游未提供的 Token 用量会显示为未知，不能据此进行完整计费核对。

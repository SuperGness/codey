...
- 线路与模型：统一管理官方账号和第三方服务，按供应商集中展示线路信息和模型，可直接编辑、同步、设置默认模型及拖动排序，支持多线路同时使用、按需停用线路、模型同步与手动维护、在同一输入框中搜索或添加模型、全选或取消全选当前搜索结果、逐模型配置上下文窗口、自动压缩阈值和输出预留，兼容原有 1M 设置，支持全局默认模型和在同一任务中切换模型。关闭本地路由后，Codex 会直接使用当前线路，Codey 保留模型管理和旧任务的模型兼容。

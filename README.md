...
- 线路与模型：统一管理官方账号和第三方服务，支持多线路、上游请求头覆盖、线路级上游代理（可指定出口地区）、模型同步、默认模型、任务内切换、Fast 选项及第三方模型上下文设置。
- 请求日志：查看请求耗时、Token 用量、缓存和错误信息，支持筛选、统计与历史清理。
- 官方账号额度：显示套餐、剩余额度和重置时间，并结合请求记录估算周限与费用。
- 个人资料统计：Codex 个人资料页的 Token 用量统计会把经 Codey 路由产生的用量一并计入，官方与路由用量汇总展示；可在控制台开关，关闭后恢复仅显示官方口径。

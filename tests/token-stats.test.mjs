...
test("observes turn/start requests and renders a footer below the turn row", async () => {
  ...
  assert.match(footer.textContent, /首字 600ms/);
  ...
  assert.match(footer.textContent, /耗时 2\.7s/);
...
test("does not render a footer when token stats are unavailable", async () => {
  const env = createEnvironment({
  ...
  assert.equal(row.children.length, 0, "no placeholder footer should be rendered");
});

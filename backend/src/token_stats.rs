...
pub async fn token_stats_snapshot(state: &Arc<AppState>, payload: &Value) -> Value {
    ...
    match stats {
        Some(stats) => {
            ...
            // Only run the (relatively expensive) subagent rollup once the turn
            // is actually complete. Polling a still-streaming turn must stay a
            // cheap lookup, otherwise every 2-second retry re-scans SQLite and
            // every child rollout and starves the bridge.
            let subagent_json = stats
                .duration_ms
                .is_some()
                .then(|| {
                    let parent_window = stats.completed_at.map(|end| {
                        let start = end - (stats.duration_ms.unwrap_or(0) as i64) / 1000;
                        (start, end)
                    });

...
- CodeyRuntime core 内置 curated remote 插件市场快照；用户显式触发修复时先解压到临时目录，校验清单并把 Codex 会忽略的 `openai-curated-remote` 保留名改为 `codey-curated`，再通过带回滚备份的目录替换安装到 `.tmp/plugins-remote`，最后用共享 `ConfigManager` 注册。修复不依赖网络，状态查询保持只读；迁移只删除指向 Codey 托管目录的旧保留名配置，用户自建的同名配置保持不变。旧 `.tmp/plugins` 官方缓存仅用于页面兼容，不再作为市场就绪条件；本地工具插件市场继续通过同一核心逻辑注册，页面层合并可用插件并清理隐藏/远程路径字段。内置快照对应 Codex++ 公开仓库的 `assets/plugin-marketplaces/openai-curated-remote.zip`，当前 SHA-256 为 `7211ea4b6d6921ed45fc7fec557906c0633f4f46359ce76d11961f1f3e88a666`；更新快照时必须同步验证 ZIP 完整性、清单改名和离线安装测试。

...
- `localRouterEnabled` 使用 `default_true` 反序列化，旧配置与默认配置继续启用本地路由。关闭时启动链路不创建 `LocalRouter`、不安装 `codey_router` 运行表、不刷新 Codey 模型目录、不执行路由专用的会话 Provider 同步，也不注入 renderer 模型路由脚本；Codex 沿用持久配置中的原生 provider/model/catalog，其他进程级增强仍按原设置构造覆盖。无租约恢复使用只读分支，不修复或写回 `config.toml`；若发现上一轮确实应用过本地路由的遗留租约，仍完成一次 Codey-owned 回滚以避免保留失效的 loopback 配置。关闭状态跳过官方线路派生持久化、首次线路导入和启动路由模型同步；线路连接字段、线路增删、启用模型和默认模型等写 API 在 revision 或网络操作前统一拒绝，唯一例外是当前 Codex Provider 的模型目录同步：第三方线路只更新该 Provider 的原始模型缓存，官方线路重新读取原生目录，两者都会按当前 Provider 重算并热更新子代理模型，且不会改写 profile、选择模型或 `config.toml`。设置加载和保存响应同样以原生当前 Provider 生成单线路模型状态，子代理配置在关闭态使用上游模型 ID；通用设置保存只允许修改非线路字段以及单独重新开启开关。运行中切换该字段标记重启要求，关闭后的 Codey 路由提示词优化返回明确错误，手动模式不受影响。

...
- Codey 配置与 Codex 配置分开保存。用户 Codex 配置原则上只读，只允许维护 Codey 自有的路由恢复桩、保留用户已开启的旧版电脑操作服务兼容入口，以及清理旧版 Codey 遗留项。
...
- config.toml 在启动准备和正式启动前做快照复核。除 Codey 自有 codey_router 恢复桩、下述 Computer Use 兼容入口和明确识别的旧版污染外，不改写用户 Provider、MCP、模型或未知字段。
...
macOS 在本地路由和原生线路共用的 `apply_isolated_runtime_router_config` 入口、读取运行配置快照之前调用 `computer_use::preserve_enabled_legacy_server`。仅当统一插件已启用、旧 `computer-use` MCP 被用户显式开启、命令与参数匹配官方旧客户端且客户端文件存在时，使用 ConfigManager 的版本校验原子写入独立的 `codey_computer_use` 条目；保留原服务配置及工具限制，将命令和工作目录改为完整路径。Codex 对 `computer-use` 的磁盘和会话配置同步不会覆盖独立条目。已有同名条目（包括手动禁用或自定义配置）不覆盖，插件明确禁用、旧服务关闭或缺失、旧版 Codex 尚未启用统一插件时不创建。由于旧服务的 `enabled=false` 无法区分自动重置与用户关闭，首次迁移前需要用户明确开启旧服务；不根据这个值自动扩大启用范围。兼容入口会保留在用户配置中，退出 Codey 时不删除。
